#!/bin/bash
#Usage: in Path: /opt/ti-processor-sdk-linux-am335x-evm-06.03.00.106/bin
#	#./create-sdcard.sh
...
THEPWD=$EXEPATH
PARSEPATH=`echo $THEPWD | grep -o '.*ti-processor-sdk-linux-am335x-evm-06.03.00.106/'`
...
sudo dd if=/opt/ti-processor-sdk-linux-am335x-evm-06.03.00.106/board-support/u-boot-2019.01+gitAUTOINC+333c3e72d3-g333c3e72d3/am335x_evm/MLO of=${DRIVE} count=1 seek=1 bs=128k
sudo dd if=/opt/ti-processor-sdk-linux-am335x-evm-06.03.00.106/board-support/u-boot-2019.01+gitAUTOINC+333c3e72d3-g333c3e72d3/am335x_evm/u-boot.img of=${DRIVE} count=2 seek=1 bs=384k
...
if [ $FILEPATHOPTION -eq 1 ] ; then
	...
	THEEVMSDK=`echo $PARSEPATH | grep -o 'ti-processor-sdk-linux-am335x-evm-06.03.00.106'`
	...
	ENTERCORRECTLY=0
		...
		do
			...
			then
				echo "Directory exists"
				echo ""
				PARSEPATH=`echo $SDKFILEPATH | grep -o '.*ti-processor-sdk-linux-am335x-evm-06.03.00.106/'`
				#echo $PARSEPATH
				...
				THEEVMSDK=`echo $SDKFILEPATH | grep -o 'ti-processor-sdk-linux-am335x-evm-06.03.00.106'`
...
EOM
	...
	while [ $ENTERCORRECTLY -ne 1 ]
	do
		BOOTUSERFILEPATH=/opt/ti-processor-sdk-linux-am335x-evm-06.03.00.106/board-support/u-boot-2019.01+gitAUTOINC+333c3e72d3-g333c3e72d3/am335x_evm
...
ENTERCORRECTLY=1
echo "Choosing a directory that contains the kernel files to be used";
KERNELFILESOPTION=2
...
EOM
	...
	do
		ENTERCORRECTLY=1
		KERNELUSERFILEPATH=/home/yong/wl8-build/build-utilites/fs/boot
...
EOM
	...
	while [ $ENTERCORRECTLY -ne 1 ]
	do
		ROOTFSUSERFILEPATH=/opt/ti-processor-sdk-linux-am335x-evm-06.03.00.106/filesystem/arago-tiny-image-am335x-evm.tar.xz		
...
cp -r /home/yong/wl8-build/build-utilites/fs/* $PATH_TO_SDROOTFS/

chown root:root $PATH_TO_SDROOTFS/
chmod -R 755 $PATH_TO_SDROOTFS/
